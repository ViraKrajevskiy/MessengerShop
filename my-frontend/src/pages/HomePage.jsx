...
export default function HomePage() {
  ...
  const maxPostsPage = Math.max(0, Math.ceil(homePosts.length / POSTS_PER_PAGE) - 1)
          ...
          <section className="home-posts-section">
            ...
            {loadingPosts ? (
              ...
              <div className="home-posts-carousel">
                ...
                <div className="home-posts-carousel__track">
                  {homePosts.slice(postsPage * POSTS_PER_PAGE, (postsPage + 1) * POSTS_PER_PAGE).map(p => (
                    <PostCard key={p.id} post={p} />
                  ))}
                </div>
                <button
                  className="home-posts-carousel__arrow"
                  onClick={() => setPostsPage(p => Math.min(maxPostsPage, p + 1))}
                  disabled={postsPage >= maxPostsPage}
                >
