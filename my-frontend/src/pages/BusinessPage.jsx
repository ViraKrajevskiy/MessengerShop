...
function InfoTabs({ biz, categoryIcon, faq, navigate, audioUrl }) {
  const { t } = useLanguage()
  ...
  return (
    <section className="bp__card">
      ...
      {tab === 'about' && (
        <div className="bp__about-tab">
          {audioUrl && <BusinessAudioPlayer audioUrl={audioUrl} />}
          {biz.description
            ? <p className="bp__about-text" style={{ marginTop: audioUrl ? 14 : 0 }}>{biz.description}</p>
            : !audioUrl && (
              <div className="bp__services-empty">
                ...
                <p>{t('biz_noDescription') || 'Описание не добавлено'}</p>
              </div>
            )
          }
          {(biz.description || audioUrl) && <div className="bp__about-divider" />}
