...
const CATEGORIES = ['Красота и уход', 'Здоровье', 'Недвижимость', 'Образование', 'Финансы', 'Юридические услуги', 'Бизнес', 'Другое']
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', confirm: '', city: '', category: '', agree: false })
      ...
      <div className="auth-page__center">
        <div className="auth-card auth-card--wide">
          ...
          <div className="auth-steps">
            <div className={`auth-steps__item ${step >= 1 ? 'auth-steps__item--active' : ''} ${step > 1 ? 'auth-steps__item--done' : ''}`}>
            ...
            <div className={`auth-steps__item ${step >= 2 ? 'auth-steps__item--active' : ''}`}>
              <div className="auth-steps__dot">2</div>
              <span>Профиль</span>
            </div>
          </div>
            ...
            <div className="auth-card__form">
              ...
              <div className="auth-field">
                ...
                <div className="auth-field__wrap">
                  ...
                  <input className={`auth-field__input ${errors.name ? 'auth-field__input--error' : ''}`}
                    type="text" name="name" placeholder="Ваше имя" value={form.name} onChange={handleChange} />
                </div>
              ...
              <div className="auth-field">
                ...
                <div className="auth-field__wrap">
                  ...
                  <input className={`auth-field__input ${errors.email ? 'auth-field__input--error' : ''}`}
                    type="email" name="email" placeholder="your@email.com" value={form.email} onChange={handleChange} autoComplete="email" />
                </div>
          ...
          {step === 2 && (
            <form className="auth-card__form" onSubmit={handleSubmit}>
              ...
              {/* Category */}
              <div className="auth-field">
                <label className="auth-field__label">Категория деятельности</label>
                <div className="auth-field__wrap">
                  <span className="auth-field__icon">
                  ...
                  <select className={`auth-field__input auth-field__input--select ${errors.category ? 'auth-field__input--error' : ''}`}
                    name="category" value={form.category} onChange={handleChange}>
                    <option value="">Выберите категорию</option>
                    {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                {errors.category && <span className="auth-field__error">{errors.category}</span>}
              </div>
